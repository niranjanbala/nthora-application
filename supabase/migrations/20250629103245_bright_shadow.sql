/*
  ...
  1. Changes
    - Add `is_demo` boolean column to `questions` table with default false
    - Create index on `is_demo` column for efficient filtering
    - Migrate existing demo_questions data to questions table with is_demo=true
    - Update RLS policies to handle demo questions appropriately

  2. Security
    - Maintain existing RLS policies
    - Allow public read access to demo questions
*/
...
CREATE POLICY IF NOT EXISTS "Allow public read for demo questions"
  ON questions
  FOR SELECT
