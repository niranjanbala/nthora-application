...
-- First, get a valid user ID to use as asker_id for demo questions
DO $$
DECLARE
  valid_user_id uuid;
BEGIN
  ...
  -- If no users exist, we'll skip the migration
  IF valid_user_id IS NOT NULL THEN
    ...
    SELECT 
      id,
      valid_user_id as asker_id, -- Use a valid user ID instead of demo question ID
      title,
      content,
      primary_tags,
      secondary_tags,
...
CREATE POLICY "Users can view network activity feed"
  ...
  USING (
    is_demo = false AND (
      ...
      (asker_id IN (SELECT get_network_user_ids.user_id FROM get_network_user_ids(auth.uid(), 2) get_network_user_ids(user_id, degree))) OR 
      (EXISTS (SELECT 1 FROM question_matches WHERE ((question_matches.question_id = questions.id) AND (question_matches.expert_id = auth.uid()))))
    )
  );
...
CREATE POLICY "Users can view questions they're matched to or asked"
  ...
  USING (
    is_demo = false AND (
      (asker_id = auth.uid()) OR 
      (EXISTS (SELECT 1 FROM question_matches WHERE ((question_matches.question_id = questions.id) AND (question_matches.expert_id = auth.uid())))) OR 
      (EXISTS (SELECT 1 FROM question_forwards WHERE ((question_forwards.question_id = questions.id) AND (question_forwards.forwarded_to = auth.uid()))))
    )
  );